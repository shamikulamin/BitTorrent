...
def server_module(socket, config):
    ...
    while (1):
        ...
        #print 'Peer 5 : New client connected ..'
        ...
        if (reqCommand == 'quit'):
            ...
            print "Peer 5: Requested bytes " , response[0], response[1], response[2]
            ...
            if (response[0] == 'upload'):
                with open(pathToSharedFolder+reqFile, 'wb') as file_to_write:
                    while True:
                        ...
                        file_to_write.close()
                        break
                print 'Peer 5 - Receive Successful'
                ...
                if ((int(response[3])-int(response[2])) <= int(maxSegmentSize)):
                    if (os.path.exists(pathToSharedFolder+response[1])):
                        with open(pathToSharedFolder+response[1], 'rb') as file_to_send:
                            ...
                            conn.sendall(data)
                            file_to_send.close()
                            print 'Peer 5 - Send Successful'
                    else:
                        print "Peer 5: Try Downloading segment from Temp folder : ","\n\n"
                        with open(pathToSharedFolder+"temp/"+response[1].split('.')[0]+"_"+response[2]+"."+response[1].split('.')[1], 'rb') as file_to_send:
                            data = file_to_send.read()
                            conn.sendall(data)
                            file_to_send.close()
                            print 'Peer 5 - Send Segment Successful'
                else: 
                    print "Peer 5 - Request segment size is greater than 1 KB. Don't send anything !! \n\n"
        conn.close()
